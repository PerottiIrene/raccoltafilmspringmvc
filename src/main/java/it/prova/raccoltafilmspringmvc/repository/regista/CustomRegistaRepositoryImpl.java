...
public class CustomRegistaRepositoryImpl implements CustomRegistaRepository {
	...
	@Override
	public Regista caricaRegistaEager(Long idRegista) {
		TypedQuery<Regista> query=entityManager
				.createQuery("select r FROM Regista r left join fetch r.films f where r.id = :idRegista", Regista.class);
		query.setParameter("idRegista", idRegista);
